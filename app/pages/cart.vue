...
const fetchCommunes = async () => {
  try {
    const res = await $fetch<{ data: Commune[] }>('http://127.0.0.1:8000/api/communes');
    communes.value = res.data || [];
  } catch (e) {
    console.error("Error loading communes from Laravel API", e);
  }
};
...
const selectedCommune = computed(() => {
  return communes.value.find(c => Number(c.id) === Number(checkoutForm.value.communeId)) || null;
});
      ...
      <div v-if="cart.length > 0" class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-4">
          ...
          <div v-if="currentStep === 1" class="space-y-4">
            ...
            <!-- CTA Button to go to Step 2 -->
            <div class="pt-4 flex justify-end">
              <button
                @click="currentStep = 2"
                class="bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 text-white font-semibold rounded-xl px-8 py-3.5 shadow-lg shadow-orange-500/10 hover-lift transition-all flex items-center space-x-2 text-xs cursor-pointer"
              >
                <span>Continuar al Despacho</span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-3.5 h-3.5 animate-pulse">
          ...
          <div v-if="currentStep === 2" class="glassmorphism p-6 rounded-2xl border border-slate-900 space-y-4">
            ...
            <form @submit.prevent="handleCheckout" class="space-y-4 text-xs">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                  <label class="text-slate-400 font-medium">Comuna</label>
                  <select
                    required
                    v-model="checkoutForm.communeId"
                    class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-slate-200 focus:outline-none focus:border-orange-500 transition-all cursor-pointer"
                  >
                    ...
                    <option
                      ...
                      :value="commune.id"
                    >
                      {{ commune.name }} (+${{ commune.shippingPrice.toLocaleString('es-CL', { minimumFractionDigits: 0 }) }})
                    </option>
                  </select>
                </div>
              </div>
          ...
          <div class="glassmorphism p-6 rounded-2xl border border-slate-900 space-y-4">
            ...
            <div class="space-y-2 text-xs">
              ...
              <div class="flex justify-between text-sm font-bold">
                ...
                <span class="text-orange-400 font-display">
                  ${{ (currentStep === 1 ? cartTotalPrice : cartFinalTotal).toFixed(2) }}
                </span>
              </div>
            </div>
          </div>
